...
function App() {
  ...
  const [tasks, setTasks] = useState<Task[]>(() => {
    const savedTasks = localStorage.getItem('campusHelp-tasks');
    return savedTasks ? JSON.parse(savedTasks) : sampleTasks;
  });

  useEffect(() => {
    localStorage.setItem('campusHelp-tasks', JSON.stringify(tasks));
  }, [tasks]);
  ...
  const handleAddTask = (newTask: Omit<Task, 'id' | 'status' | 'createdAt'>) => {
    const task: Task = {
      ...newTask,
